...
export default function Navbar({ lang, setLang, onOpenContact }: NavbarProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Transition topnav after scrolling past hero section (~450px)
  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none p-4 md:p-6 transition-all duration-500">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <AnimatePresence mode="wait">
            {!isScrolled ? (
              ...
              /* State 2: Minimal Fixed Topnav Bar (Scrolled Past Hero) */
              <motion.div
                key="minimal-nav"
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div
                  ...
                  className="pointer-events-auto cursor-pointer flex items-center gap-2 bg-[#181818]/90 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-white/10 shadow-lg"
                >
                  <span className="font-sans font-bold text-white text-sm md:text-base">
                    Khanhtruong Nguyen
                  </span>
                  <span className="font-mono text-[#C6A85B] text-sm md:text-base font-semibold">
                    Portfolio
                  </span>
                </div>

                {/* Right side compact pill: Contact + Hamburger Menu */}
                <div className="pointer-events-auto bg-[#242424]/90 backdrop-blur-md border border-white/10 rounded-2xl px-3 py-1.5 flex items-center gap-2 shadow-xl">
                  <button
                    onClick={onOpenContact}
                    className="px-4 py-1.5 text-sm font-sans font-medium text-white hover:text-[#00DC6C] transition-colors cursor-pointer"
                  >
                    Contact
                  </button>
                  <button
                    onClick={() => setMobileMenuOpen(true)}
                    className="p-2 bg-[#1A1A1A] hover:bg-[#333] rounded-xl text-white transition-colors cursor-pointer flex items-center justify-center border border-white/5"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            <div className="flex flex-col gap-6 my-auto font-mono text-3xl md:text-5xl font-bold">
              <button
                onClick={() => scrollToSection("hero")}
            ...
            <div className="flex justify-between items-center text-sm font-mono text-white/50 border-t border-white/10 pt-6">
