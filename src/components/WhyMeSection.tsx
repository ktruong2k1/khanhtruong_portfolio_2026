...
export default function WhyMeSection({ lang, onOpenContact }: WhyMeSectionProps) {
  ...
  return (
    <section
      id="why-me"
      className="w-full min-h-screen lg:h-screen snap-start snap-always bg-[#121212] text-white border-b border-white/5 relative overflow-hidden flex flex-col justify-center px-5 sm:px-6 md:px-12 lg:px-[10vh] py-10 sm:py-16 scroll-mt-0"
    >
      <div className="max-w-[1440px] mx-auto space-y-6 sm:space-y-12 lg:space-y-16 w-full my-auto">
        {/* Top Header & Paragraph - Aligned exactly with 3-Column Stats Grid below */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8 lg:gap-12 items-start w-full">
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-8 lg:gap-12 pt-4 sm:pt-8 w-full">
          {stats.map((stat, idx) => (
            <motion.div
              key={idx}
